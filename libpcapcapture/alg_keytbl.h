...
//function prototypes;
key_tbl_t* key_tbl_init(int n, int tbl_list_size, int tbl_list_len, double entry_to);
...
key_tbl_entry_p_t key_tbl_record(key_tbl_t* tbl, tuple_t* p, double ts);
key_tbl_entry_p_t key_tbl_find(key_tbl_t* tbl, tuple_t* p);

long long key_tbl_read_file(char* filename, key_tbl_t* tbl);

void read_rb(const char* dir, const char* alg, int interval, key_tbl_t* tbl_rb, long long* rb_tot, 
    long long* max_perflow_dec, long long* all_flows_dec, int* rb_card);
int read_rb_multi(const char* dir, const char* alg, int interval, key_tbl_t* tbl_rb, long long* rb_tot, 
...
// key_tbl_entry_p_t key_tbl_record(key_tbl_t* tbl, unsigned char* key, double ts);
