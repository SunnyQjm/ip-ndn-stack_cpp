 ...
 */
 ...
 struct RBMeta {
     ...
     int localReadIndex; //write start
     int nextWriteIndex;
     int wBatchCount; //write end;
 ...
 static inline int nextVal(int x) {return (x+1) >= RB_SIZE ? 0 : x+1; }  //get the serial nubber

 ringbuffer_t* create_ringbuffer_shm1(const char* shm_name, unsigned long tuple_size);
 ringbuffer_t* create_ringbuffer_shm2(const char* shm_name, unsigned long tuple_size);
 ringbuffer_t* create_ringbuffer_shm3(const char* shm_name, unsigned long tuple_size);
 ringbuffer_t* create_ringbuffer_shm4(const char* shm_name, unsigned long tuple_size);
